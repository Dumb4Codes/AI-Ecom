...
import { getAllCategoriesQueryFn, createProductMutationFn, updateProductMutationFn, getProductByIdQueryFn, generateProductAiMutationFn } from "@/lib/api";
...
export default function AdminNewProductPage() {
  ...
  const { data: existingProductData, isLoading: productLoading } = useQuery({
  ...
  const onSubmit = (values: ProductFormValues) => {
    ...
    if (isEditMode && id) {
      updateMutation.mutate({
        productId: id,
        data: { ...values, images, discountLabel: values.discountLabel || "" },
      });
    } else {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold tracking-tight">{isEditMode ? "Edit Product" : "New Product"}</h2>
          <p className="text-muted-foreground">
            {isEditMode ? "Update this product's details." : "Add a new product to your store catalog."}
          </p>
        </div>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_400px]">
            ...
            <div className="space-y-6 lg:sticky lg:top-6 lg:self-start">
              ...
              <Button type="submit" className="w-full" size="lg" disabled={createMutation.isPending || updateMutation.isPending}>
