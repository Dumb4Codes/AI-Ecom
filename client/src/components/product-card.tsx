...
const ProductCard = ({
  ...
  return (
    <Card
      className={cn(
        "group relative gap-0 rounded-none bg-transparent shadow-none! p-0 ring-0 overflow-hidden transition-transform duration-300 hover:-translate-y-0.5",
        className
      )}
    >
        {isOutofStock && (
      ...
      <CardContent className={cn("relative  flex flex-col gap-1 p-0",
        ...
        <div className="relative aspect-square">
          ...
          >
            <img
              src={imageUrl}
              alt={name}
              className="mb-2 max-h-full max-w-full object-contain group-hover:scale-[1.04]"
            />
          </Link>
        ...
        {!isOutofStock && (
          <Button
            ...
            className="absolute right-0 top-0 h-10 rounded-full px-4 text-sm font-semibold bg-green-light! hover:shadow-md active:scale-95"
          >
